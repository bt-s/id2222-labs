...
class Triest():
    def __init__(self, Sigma: Graph, M: int):
        ...
        # Initialize the eta coefficient for when using the improved Triest
        # algorithm
        self.eta = 1
    ...
    def update_counter(self, val: int, edge: Tuple):
        ...
        Updates:
            self.tau
        """
        # Check whether the nodes exist in the graph S
        nodes = self.S.nodes()
    ...
    def triest_impr(self) -> Tuple[Graph, float]:
        """The improved base algorithm for counting graph triangles as suggested
            in the Triest paper
        ...
        Note: The differences between this algorith and triest_base() are the
              following:
                - UpdateCounters is called unconditionally for each element on
                  the stream
                - We never decrement the counters when an edge is deleted from S
                - We let UpdateCounters perform a weighted increase of the tau
                  counter using eta
        """
        # Get a list of the edges
        Sigma_e = list(self.Sigma.edges)
        ...
        for t, new_edge in enumerate(Sigma_e):
            # Compute the eta coeffecient
            self.eta = max(1, ((t-1)*(t-2))/(self.M*(self.M-1)))
